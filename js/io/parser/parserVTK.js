/**
 * Created by shusa_000 on 9/23/2015.
 */

// provides
goog.provide('DVT.parserVTK');
...
goog.inherits(DVT.parserVTK, DVT.parser);


/**
 * @inheritDoc
 */
DVT.parserVTK.prototype.parse = function(object, data, loader) {
    ...
    // buffer everything in DVT.triplets containers and then order it
    ...
    // support one kind
    this._objectType = DVT.displayable.types.TRIANGLES;
...
/**
 * Parses a line of .VTK data and modifies the given DVT.triplets containers.
 *
 * @param {!string} line The line to parse.
 * @protected
 */
DVT.parserVTK.prototype.parseLine = function(line) {
    ...
    switch (firstLineField) {
        ...
        case 'POINTS':
            ...
            this._unorderedPoints = new DVT.triplets(numberOfPoints * 3);
            this._unorderedNormals = new DVT.triplets(numberOfPoints * 3);
        ...
        case 'VERTICES':
            ...
            if (numberOfElements >= 3) {
                ...
                this._objectType = DVT.displayable.types.POINTS;
            } else {
        ...
        case 'TRIANGLE_STRIPS':
            ...
            this._pointsMode = false;
            this._pointDataMode = false;
            this._objectType = DVT.displayable.types.TRIANGLE_STRIPS;
        ...
        case 'LINES':
            ...
            this._pointsMode = false;
            this._pointDataMode = false;
            this._objectType = DVT.displayable.types.LINES;
        ...
        case 'POLYGONS':
            ...
            this._pointsMode = false;
            this._pointDataMode = false;
            this._objectType = DVT.displayable.types.POLYGONS;
    ...
    if (this._pointsMode) {
        ...
        // assume maDVT. 9 coordinate values (== 3 points) in one row
...
/**
 * Configure DVT.object points and normals. This method takes the object type into
 * consideration to f.e. use degenerated triangles for TRIANGLE_STRIPS.
 *
 * @param {!DVT.triplets} p The points container of the DVT.object.
 * @param {!DVT.triplets} n The normals container of the DVT.object.
 */
DVT.parserVTK.prototype.configure = function(p, n) {
    ...
    do {
        ...
        for (k = 0; k < currentGeometryLength; k++) {
            ...
            if (this._objectType == DVT.displayable.types.LINES &&
            ...
            else if (this._objectType == DVT.displayable.types.TRIANGLE_STRIPS) {
...
goog.exportSymbol('DVT.parserVTK', DVT.parserVTK);
goog.exportSymbol('DVT.parserVTK.prototype.parse', DVT.parserVTK.prototype.parse);
